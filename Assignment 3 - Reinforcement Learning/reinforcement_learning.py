...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        # get all the actions of the environment
        actions = env.actions()
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            # compute the utility of the next state if it is not a terminal state
            q_next = self.compute_utility(env, next_state)
        
        # update the Q(state, action) using the Q-Learning update rule
        self.Q[state][action] += self.learning_rate * (reward + self.discount_factor * q_next - self.Q[state][action])
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        #TODO: Complete this function
        q = 0
        # apply the equation from instructions to evaluate the value of q
        for i in features.keys():
            q += self.weights[action][i] * features[i]
        return q
    
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        #TODO: Complete this function
        max_utility = float('-inf')
        # iterate through all actions and find the max utility value using the given features
        for action in self.actions:
            max_utility = max(max_utility, self.__compute_q_from_features(features, action))
        return max_utility
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            # get the environment actions and compute the q value of the next state
            actions = env.actions()
            q_next = self.compute_q(env, next_state, actions[0])
            
            # for all actions, check if the action has a q value larger than the max and update current max
        ...
        # compute the q value of the current state
        q = self.compute_q(env, state, action)

        # update the weights according to the equation from the instructions
        for i in features.keys():
            self.weights[action][i] += self.learning_rate * (reward + self.discount_factor * q_next - q) * features[i]
